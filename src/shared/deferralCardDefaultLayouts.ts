...
import { DEFERRAL_CARD_CANVAS_WIDTH, DEFERRAL_CARD_PADDING, type DeferralCanvasElement } from './deferralCardCanvasSchema';
...
const CONTENT_W = W - DEFERRAL_CARD_PADDING * 2;
...
const TXADMIN_LOCALE = {
    ...
    whitelist: {
        adminOnly: {
            ...
            insufficientIds:
                'You do not have <code>discord</code> or <code>fivem</code> identifiers, and at least one of them is required to validate if you are a txAdmin administrator.',
            denyMessage: 'Your identifiers are not assigned to any txAdmin administrator.',
        },
        guildMember: {
...
function txAdminClassicLayout(opts: TxAdminClassicOpts): DeferralCardLayout {
    ...
    const height = opts.height ?? Math.max(220, y + 24);
...
export function getDefaultDeferralCardLayout(scenarioId: DeferralScenarioId): DeferralCardLayout {
    ...
    const height = Math.max(provisionalHeight, contentBottom + 24);
    return {
        ...base,
        canvas: {
            ...base.canvas!,
...
export function isPreviousStyledDeferralDefault(template: DeferralCardTemplate): boolean {
    ...
    return ids.some((id) => id.endsWith(':badge') || /:status/.test(id) || /:f\d/.test(id));
}
