...
import { DEFERRAL_CARD_WATERMARK_PATH } from './deferralCardLogo';
...
const htmlGuildNameTag = '<strong style="color: #c8c8c8">';
...
function renderLayoutBlock(input: RenderLayoutBlockInput): string {
    const { block, template, tokens, renderCtx, applyTokens } = input;
...
function canvasElementStyle(el: DeferralCanvasElement, cardContentWidth: number): string {
    ...
    if (el.type === 'rejection_message' || el.type === 'ban_reason') {
        ...
        const maxW = Math.max(48, cardContentWidth - el.x);
        const minW = Math.min(el.width ?? 64, maxW);
        parts.push(`min-width:${minW}px`);
        parts.push(`max-width:${maxW}px`);
...
export function canvasElementWrapsAtRender(el: DeferralCanvasElement, _tokens: DeferralBanTokenFields): boolean {
...
export function fitCanvasHeightForContent(
    ...
    const requiredInner = Math.max(baseInner, maxBottom + 8);
...
function buildDeferralCardFromCanvas(
    ...
    const inner = canvas.elements
        .map((element) => {
            if (element.enabled === false || element.type === 'logo') return '';
    ...
    const innerWrapper =
        ...
        `${markup}</div>`;
    return buildDeferralCardHtml('', innerWrapper, { skipTitle: true });
}
...
export function buildDeferralCardFromLayout(
    ...
    if (templateHasCanvasLayout(synced)) {
        return buildDeferralCardFromCanvas(synced, tokens, applyTokens, renderCtx);
    }
    ...
    const inner = layout.blocks
        ...
        .map((block) =>
            renderLayoutBlock({ block, template: synced, tokens, renderCtx, applyTokens }),
        )
        .filter(Boolean)
        .join('');
    return buildDeferralCardHtml('', inner, { skipTitle: true });
}
...
export function renderDeferralCardPreview(
    ...
    if (templateHasVisualLayout(template) || templateHasCanvasLayout(template)) {
        return buildDeferralCardFromLayout(template, tokenPayload, {
    ...
    return buildDeferralCardHtml(resolvedTitle, content);
}
