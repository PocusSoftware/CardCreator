...
const DeferralCardExportFileV1Schema = z.object({
    cardCreatorDeferralCard: z.literal(DEFERRAL_CARD_EXPORT_VERSION),
...
const DeferralCardExportFileV2Schema = z.object({
    cardCreatorDeferralCard: z.literal(DEFERRAL_CARD_EXPORT_VERSION_V2),
    exportedAt: z.string().optional(),
...
export function exportDeferralScenario(
    ...
    const base = {
        cardCreatorDeferralCard: version,
...
/**
 * Wrap a string in a Lua long-string literal, choosing the minimum level
 * that avoids a premature close (e.g. `]]` in the JSON).
 */
...
/** Serialize a full export as a Lua file using `json.decode`. */
export function exportDeferralCardsFullAsLua(config: DeferralCardsConfig): string {
    const payload = exportDeferralCardsFull(config);
    const json = JSON.stringify(payload);
    return `-- deferral cards export (generated)\nreturn json.decode(${luaLongString(json)})\n`;
}

/** Serialize a single-scenario export as a Lua file using `json.decode`. */
export function exportDeferralScenarioAsLua(config: DeferralCardsConfig, scenarioId: DeferralScenarioId | string): string {
    const payload = exportDeferralScenario(config, scenarioId);
    const json = JSON.stringify(payload);
    return `-- deferral cards export (generated)\nreturn json.decode(${luaLongString(json)})\n`;
}
...
export function importDeferralCardFile(
    ...
    if (!parsed.success) {
        return { ok: false, error: 'Invalid deferral card file (missing required header).' };
    }
